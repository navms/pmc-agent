...
- 父 Agent（`pmc_supervisor`）负责听懂用户、闲聊澄清、调度子 Agent、执行写操作；
...
本系统用到的标准事件包括：`RUN_STARTED` / `RUN_FINISHED` / `RUN_ERROR`、`TEXT_MESSAGE_*`、`REASONING_MESSAGE_*`、`TOOL_CALL_*` 等。
...
| Agent 续跑状态 | Harness 内部 state | 本地 JSON 文件 | 进程内续跑能力受影响 |
...
1. 用户提问；
2. 总助手开始处理（调度细节可隐藏）；
3. 出现「查询/汇总/导出/图表」嵌套卡片，卡片内有工具过程与子回复；
4. 总助手用自然语言转述要点。
...
- 写操作未接真实银行，也不落业务表。
- HITL pending 与导出索引在内存，重启丢失。
- Agent 状态是本地 JSON，不是跨实例共享存储。
...
下一篇会解释：为什么不是「一个 Agent 挂所有工具」、为什么从 Spring AI Alibaba 迁到 AgentScope、为什么写工具不能塞进子 Agent。
