...
public class ChatAgentAppService {
                    ...
                    if (nodeOutput instanceof StreamingOutput<?> streamingOutput) {
                        ...
                        if (message == null) {
                            ...
                            if (outputType == OutputType.AGENT_MODEL_FINISHED) {
                                ...
                                List<PromotedNarrative> promotedNarratives = List.of();
                                if (messageResponse instanceof ToolResponseMessageResponse toolResponse) {
                                    ...
                                    promotedNarratives = promoteAndCompactAgentToolNarratives(toolResponse);
                                }
                                responses.add(new ChatResponse(
                                        node,
                                        agentName,
                                        ...
                                        tokenUsage,
                                        chunk));
                                if (!promotedNarratives.isEmpty()) {
                                    StringBuilder joined = new StringBuilder();
                                    for (PromotedNarrative promoted : promotedNarratives) {
                                        if (!joined.isEmpty()) {
                                            joined.append("\n\n");
                                        }
                                        joined.append(promoted.text());
                                        // 按子 Agent 打标，供前端嵌套卡片归组
                                        responses.add(new ChatResponse(
                                                "_SUB_AGENT_NARRATIVE_",
                                                promoted.agentName(),
                                                new AssistantMessageResponse(promoted.text()),
                                                null,
                                                promoted.text()));
                                    }
                                    lastPromotedNarrative.set(joined.toString());
                                }
                            }
                        }
     ...
     * 按子 Agent 分别提升为助手消息，并把工具块压缩为占位。
     *
     * @param toolResponse 工具消息
     * @return 各子 Agent 需提升的自然语言；无可提升内容时返回空列表
     */
    private List<PromotedNarrative> promoteAndCompactAgentToolNarratives(ToolResponseMessageResponse toolResponse) {
        if (toolResponse == null || CollectionUtils.isEmpty(toolResponse.getResponses())) {
            return List.of();
        }
        List<PromotedNarrative> narratives = new ArrayList<>();
        for (ToolResponseMessageResponse.ToolResponse response : toolResponse.getResponses()) {
            if (response == null || !AGENT_TOOL_NAMES.contains(response.getName())) {
                continue;
            }
            ...
            narratives.add(new PromotedNarrative(response.getName(), data));
     ...
     * @param text      自然语言
     */
    private record PromotedNarrative(String agentName, String text) {
    }
    ...
    static final class StreamPersistSink {
        ...
        /** 当前缓冲中的助手消息归属（子 Agent 终答时写入） */
        private String pendingAgentName;

        /** 当前缓冲中的助手消息节点（如 _SUB_AGENT_NARRATIVE_） */
        ...
        void onResponse(ChatResponse response) {
            ...
            if ("tool-request".equals(type) || "tool".equals(type) || "tool-confirm".equals(type)) {
                ...
                persistMessage(message, response.getAgentName(), response.getNode(), forThis);
                if (forThis != null) {
                    pendingUsage = null;
                }
                return;
            ...
            if ("assistant".equals(type)) {
                // 子 Agent 终答单独落库，不与 Supervisor 流式缓冲混写
                if ("_SUB_AGENT_NARRATIVE_".equals(response.getNode())) {
                    flushAssistant();
                    String content = StringUtils.hasText(response.getChunk())
                            ? response.getChunk()
                            : message.getContent();
                    AssistantMessageResponse narrative =
                            new AssistantMessageResponse(content == null ? "" : content);
                    persistMessage(
                            narrative,
                            response.getAgentName(),
                            response.getNode(),
                            takePendingUsage(response.getTokenUsage()));
                    return;
                }
                rememberUsage(response.getTokenUsage());
                if (StringUtils.hasText(response.getChunk())) {
                    pendingAgentName = response.getAgentName();
                    pendingNode = response.getNode();
                    appendChunk(response.getChunk());
        ...
        private void persistMessage(
                MessageResponse message, String agentName, String node, TokenUsageVO tokenUsage) {
        ...
        private TokenUsageVO flushAssistant() {
            ...
            AssistantMessageResponse message = new AssistantMessageResponse(assistant.toString());
            persistMessage(message, pendingAgentName, pendingNode, usage);
