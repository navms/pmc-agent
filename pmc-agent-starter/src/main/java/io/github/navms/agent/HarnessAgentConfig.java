...
public class HarnessAgentConfig {
    ...
    public HarnessAgent supervisorAgent(
        ...
        HarnessAgent supervisor = HarnessAgent.builder()
                ...
                .sysPrompt(SUPERVISOR_PROMPT.formatted(today(), "何锦"))
                .model(chatModel)
                .toolkit(toolkit)
                .distributedStore(distributedStore)
                ...
                .subagent(getSubagentDeclaration("query_bank", """
                        ...
                        """, QUERY_PROMPT.formatted(today(), "何锦")))
                .subagent(getSubagentDeclaration("summarize_bank", """
                        对交易明细、支付单、支付明细单、账户、电子回单、电子对账单做 SQL 汇总：合计金额、笔数、按账号/日/月/状态等分组。
                        适用于“一共多少”“合计”“占比”“按月统计”。
                        """, SUMMARY_PROMPT.formatted(today(), "何锦")))
                .subagent(getSubagentDeclaration("export_excel", """
                        将账户、交易明细、支付单或汇总结果导出为 Excel，返回下载地址。
                        用户说导出、下载表格、做成 Excel 时使用。
                        """, EXCEL_PROMPT.formatted(today(), "何锦")))
                .subagent(getSubagentDeclaration("create_chart", """
                        根据银企汇总数据生成柱状图、折线图或饼图。
                        用户说图表、趋势、对比、可视化时使用。
                        """, CHART_PROMPT.formatted(today(), "何锦")))
                .subagentFactory("query_bank", name -> getReActAgent(name, QUERY_PROMPT.formatted(today(), "何锦"), chatModel, bankQueryTools))
                .subagentFactory("export_excel", name -> getReActAgent(name, EXCEL_PROMPT.formatted(today(), "何锦"), chatModel, excelExportTools))
                .subagentFactory("create_chart", name -> getReActAgent(name, CHART_PROMPT.formatted(today(), "何锦"), chatModel, chartTools))
                .subagentFactory("summarize_bank", name -> getReActAgent(name, SUMMARY_PROMPT.formatted(today(), "何锦"), chatModel, bankAggregateTools))
                .build();
